    ...
    <section class="content">
        <div class="box">
            <div class="box-header">
                <div class="pull-left">
                    <h3 class="box-title">Documentos de la Orden de Compra <a href="{{ route('ordenes_compra.index') }}">{{ $orden_compra->numero_factura }}</a></h3>
                </div>
                <div class="pull-right">
                    <a href="{{ route('documentos.ordenes_compra.create', $orden_compra->id) }}" class="btn btn-primary">Agregar documento</a>
                </div>
            </div>
            <div class="box-body table-responsive">
                <table id="tabla" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->ordenCompra->numero_factura }}</td>
                            <td>{{ $item->archivo }}</td>
                            <td>
                                <table>
                                    <tbody><tr>
...
function eliminateHandle(id){
    ...
    if (confirm(text) == true) {
        try {
            ...
            fetch("/documentosordenescompra/{{ $orden_compra->id }}/"+id, 
            ...
            .then((data) => {
                if(data.status == "success"){
                    location.href = "{{ route('documentos.ordenes_compra.index', $orden_compra->id) }}";
