    ...
    <section class="content">
        <div class="box">
            <div class="box-header">
                <div class="pull-left">
                    <h3 class="box-title">Documentos de la Licitación <a href="{{ route('proyectos.licitaciones.index', $licitacion->proyecto_id) }}">{{ $licitacion->concepto }}</a></h3>
                </div>
                <div class="pull-right">
                    <a href="{{ route('proyectos.documentoslicitaciones.create', $licitacion->id) }}" class="btn btn-primary">Agregar documento</a>
                </div>
            </div>
...
function eliminateHandle(id){
    ...
    if (confirm(text) == true) {
        try {
            ...
            fetch("/documentoslicitacionesproyectos/{{ $licitacion->id }}/"+id, 
            ...
            .then((data) => {
                if(data.status == "success"){
                    location.href = "{{ route('proyectos.documentoslicitaciones.index', $licitacion->id) }}";
